const requestTracker = {
  ...
  userIdToRequestCount: new Map<
    string,
    { count: number; lockoutStart?: number }
  >(),
  windowStart: 0,
};
...
export const rateLimit = (
  ...
  if (
    userRequestData &&
    userRequestData.lockoutStart &&
    now - userRequestData.lockoutStart < lockoutPeriod
  ) {
    return {
      error: `Too many attempts, please try again in ${Math.ceil(
        (lockoutPeriod - (now - userRequestData.lockoutStart)) / 1000
      )} seconds.`,
    };
  }
  ...
  if (currentRequestCount >= maxRequests) {
    requestCountMap.set(identifier, {
